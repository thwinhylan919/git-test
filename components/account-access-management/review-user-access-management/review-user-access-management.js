define([
    ...
    return function(rootParams) {
        ...
        if (self.rootParams && self.rootParams.userAccountAccessDTOs) {
            ...
            for (let z = 0; z < self.rootParams.userAccountAccessDTOs().length; z++) {
                ...
                if (self.rootParams.userAccountAccessDTOs()[z].accountType() === "LER") {
                    if (self.rootParams.userAccountAccessDTOs()[z].accessStatus() === true) {
                        self.selectedLMPolicy = ko.observable("lmAuto");
                        self.selectedLMPolicyChecked = ko.observable(["lmAuto"]);
                    } else {
                        self.selectedLMPolicy = ko.observable("lmManual");
                        self.selectedLMPolicyChecked = ko.observable(["lmManual"]);
                    }
                }

                if (self.rootParams.userAccountAccessDTOs()[z].accountType() === "VRA") {
                    if (self.rootParams.userAccountAccessDTOs()[z].accessStatus() === true) {
                        self.selectedVirtualPolicy = ko.observable("vraAuto");
                        self.selectedVirtualPolicyChecked = ko.observable(["vraAuto"]);

                    } else {
                        self.selectedVirtualPolicy = ko.observable("vraManual");
                        self.selectedVirtualPolicyChecked = ko.observable(["vraManual"]);

                    }
                }

                if (self.rootParams.userAccountAccessDTOs()[z].accountType() === "VER") {
                    if (self.rootParams.userAccountAccessDTOs()[z].accessStatus() === true) {
                        self.selectedVamEnabledRealAccPolicy = ko.observable("verAuto");
                        self.selectedVERPolicyChecked = ko.observable(["verAuto"]);

                    } else {
                        self.selectedVamEnabledRealAccPolicy = ko.observable("verManual");
                        self.selectedVERPolicyChecked = ko.observable(["verManual"]);
        ...
        self.populateSelectedAccounts = function() {
            ...
            if (self.rootParams && self.rootParams.userAccountAccessDTOs) {
                if (self.rootParams.userAccountAccessDTOs().length > 0) {
                    for (let x = 0; x < self.rootParams.userAccountAccessDTOs().length; x++) {
                        if (self.rootParams.userAccountAccessDTOs()[x].accountExclusionDTOs()) {
                            ...
                            } else if (self.rootParams.userAccountAccessDTOs()[x].accountType() === "LER") {
                                self.isLMAllowed(self.rootParams.userAccountAccessDTOs()[x].accessStatus());

                                if (self.rootParams.userAccountAccessDTOs()[x].accountAccessId) {
                                    self.lmAccountAccessId(self.rootParams.userAccountAccessDTOs()[x].accountAccessId());
                                }

                                self.updatedLMExclusionNumberList(self.rootParams.userAccountAccessDTOs()[x].accountExclusionDTOs());
                            ...
                            } else if (self.rootParams.userAccountAccessDTOs()[x].accountType() === "VER") {
                                self.isVAMEnabledRealAllowed(self.rootParams.userAccountAccessDTOs()[x].accessStatus());

                                if (self.rootParams.userAccountAccessDTOs()[x].accountAccessId) {
                                    self.vamEnabledRealAccountAccessId(self.rootParams.userAccountAccessDTOs()[x].accountAccessId());
                                }

                                self.updatedVAMEnabledRealExclusionNumberList(self.rootParams.userAccountAccessDTOs()[x].accountExclusionDTOs());
                            ...
                            } else if (self.rootParams.userAccountAccessDTOs()[x].accountType() === "VRA") {
                                self.isVirtualAllowed(self.rootParams.userAccountAccessDTOs()[x].accessStatus());

                                if (self.rootParams.userAccountAccessDTOs()[x].accountAccessId) {
                                    self.virtualAccountAccessId(self.rootParams.userAccountAccessDTOs()[x].accountAccessId());
                                }

                                self.updatedVirtualExclusionNumberList(self.rootParams.userAccountAccessDTOs()[x].accountExclusionDTOs());
        ...
        self.createDataSource = function() {
            ...
            for (let j = 0; j < self.selectedLMAccounts().length; j++) {
            ...
            for (let j = 0; j < self.selectedVirtualAccounts().length; j++) {
            ...
            if (self.showReviewScreen() === true) {
                ...
                self.lmData = $.map(ko.utils.unwrapObservable(self.reviewLMList()), function(val) {
                    val.attr = {
                        accountNumber: {
                            displayValue: val.accountNumber.displayValue,
                            value: val.accountNumber.value
                ...
                self.lmReviewDatasource = new oj.FlattenedTreeTableDataSource(new oj.FlattenedTreeDataSource(new oj.JsonTreeDataSource(self.lmData), options));
                self.reloadLMTable(true);

                self.verData = $.map(ko.utils.unwrapObservable(self.reviewVERList()), function(val) {
                    val.attr = {
                        accountNumber: {
                            displayValue: val.accountNumber.displayValue,
                            value: val.accountNumber.value
                ...
                self.verReviewDatasource = new oj.FlattenedTreeTableDataSource(new oj.FlattenedTreeDataSource(new oj.JsonTreeDataSource(self.verData), options));
                self.reloadVERTable(true);

                self.vraData = $.map(ko.utils.unwrapObservable(self.reviewVRAList()), function(val) {
                    val.attr = {
                        accountNumber: {
                            displayValue: val.accountNumber.displayValue,
                            value: val.accountNumber.value
                ...
                self.vraReviewDatasource = new oj.FlattenedTreeTableDataSource(new oj.FlattenedTreeDataSource(new oj.JsonTreeDataSource(self.vraData), options));
