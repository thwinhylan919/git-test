define([
  ...
  return function (params) {
    ...
    CorpDetailsModel.allPartyDetails().then(function (data) {
      ...
      params.rootModel.productData().data.type = "New Facility Application";
      params.rootModel.productData().data.partyId = self.selectedParty();

      if(self.productData().payload.facilityDetailsModel){
      params.rootModel.productData().data.amount = self.productData().payload.facilityDetailsModel.facilityAmount();
      params.rootModel.productData().data.currency = self.productData().payload.facilityDetailsModel.facilityCurrency();
      }

      if (params.rootModel.productData().payload !== undefined) {
        params.rootModel.productData().payload.partyId = self.selectedParty();
      }
