define([
  ...
  return function(rootParams) {
    ...
    self.maxAddonCardsAllowed = ko.observable(self.productDetails().offers.standardFeatureDTO.maximumAddOnCard);
    ...
    self.getAddonDetails = function() {
      CardAddonModelObject.fetchAddOnCardDetails(self.productDetails().submissionId.value, self.productDetails().facilityId, self.productDetails().simulationId(), self.productDetails().offers.offerId).done(function(data) {
        if (data.addOnCardHolders && data.addOnCardHolders.length > 0) {
          ...
          self.applicantDetails()[0].creditCardInfo.customizeCard.addonCards(self.addonCards());
    ...
    self.submitAddOn = function(index) {
      const cardAddOnTracker = document.getElementById("cardAddOnTracker");

      if (cardAddOnTracker===null || cardAddOnTracker.valid === "valid") {
      ...
      cardAddOnTracker.showMessages();
      cardAddOnTracker.focusOn("@firstInvalidShown");
    }
    };
    ...
    self.initializeModel = function() {
      ...
      if (self.usedAddonCards() === 0) {
        ...
        self.addonCards().push(card);
      }else if (self.usedAddonCards() < self.maxAddonCardsAllowed()) {
          self.showAdd(true);
        } else {
          self.showAdd(false);
        }
