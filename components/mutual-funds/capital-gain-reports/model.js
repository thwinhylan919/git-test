define([
  ...
  /**
   * Main file for Capital Gain Reports Model. This file contains the model definition
   * for list of moduleType and data fetched from the server through REST call.<br/><br/>
   * The injected Model Class will have below properties:
   *
   * @namespace
   * @class CapitalGainReportsModel
   */
  const CapitalGainReportsModel = function () {
    ...
    let capitalGainLossReportDeferred;

    /**
     * Private method to fetch capital gain report details based on available parameters.
     * This method will resolve a passed deferred object which can be returned from calling function to the parent.
     * @function fetchCapitalGainReports
     * @memberOf ErrorModel
     * @param {String} investmentAccount - payload to pass
     * @param {String} fundHouseCode - An object type String
     * @param {String} schemeCode -  An object type String
     * @param {String} financialYear - An object type String
     * @param {String} capitalGainLossType - An object type String
     * @param {Object} deferred - An object type deferred
     * @returns {void}
     * @private
     */

    const fetchCapitalGainReports = function (investmentAccount, fundHouseCode, schemeCode, financialYear, capitalGainLossType, deferred) {
      const
        options = {
          url: "accounts/investmentAccounts/{investmentAccountId}/capitalGainReports?fundHouseCode={fundHouseCode}&schemeCode={schemeCode}&financialYear={financialYear}&capitalGainLossType={capitalGainLossType}",
          success: function (data) {
            deferred.resolve(data);
          },
          error: function (data) {
        ...
        params = {
          ...
          capitalGainLossType: capitalGainLossType
        };
    ...
    /**
     * Private method to fetch Capital Gain report details in PDF format.
     * This method will resolve a passed deferred object which can be returned from calling function to the parent.
     *
     * @function fetchPDF
     * @memberOf ErrorModel
     ...
     * @private
     */
    const fetchPDF = function (investmentAccount, fundHouseCode, schemeCode, financialYear, capitalGainLossType, deferred) {
      const
        options = {
          url: "accounts/investmentAccounts/{investmentAccountId}/capitalGainReports?fundHouseCode={fundHouseCode}&schemeCode={schemeCode}&financialYear={financialYear}&capitalGainLossType={capitalGainLossType}&media=application/pdf&mediaFormat=pdf",
          success: function (data) {
            deferred.resolve(data);
          },
          error: function (data) {
    ...
    return {
       ...
       * @returns {Object} - deferredObject
       * @example
       *       CapitalGainReportsModel.fetchCapitalGainLossTypes().done(function(data) {
       *
       *       });
       */

      fetchCapitalGainReports: function (investmentAccountId, fundHouseCode, schemeCode, financialYear, capitalGainLossType) {
        capitalGainLossReportDeferred = $.Deferred();
        fetchCapitalGainReports(investmentAccountId, fundHouseCode, schemeCode, financialYear, capitalGainLossType, capitalGainLossReportDeferred);

        return capitalGainLossReportDeferred;
      },
       ...
       *       });
       */
      fetchPDF: function (investmentAccountId, fundHouseCode, schemeCode, financialYear, capitalGainLossType) {
        fetchPDFDeferred = $.Deferred();
        fetchPDF(investmentAccountId, fundHouseCode, schemeCode, financialYear, capitalGainLossType, fetchPDFDeferred);
  ...
  return new CapitalGainReportsModel();
});