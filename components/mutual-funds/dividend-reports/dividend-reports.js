define([
  ...
  "ojs/ojtoolbar",
  "ojs/ojswitch"
], function(oj, ko, $, ResourceBundle, DetailedDividendReport) {
  "use strict";
  ...
  return function(Params) {
    ...
    Params.dashboard.headerName(self.resource.dividendHeader);
    ...
    DetailedDividendReport.fetchInvestmentAccounts().done(function(data) {
      if (data.investmentAccounts.length) {
        ...
        self.headerText = ko.observableArray([{
          ...
          {
            headerText: self.resource.dividendAmount,
        ...
        self.valueChange = function() {
          ...
          DetailedDividendReport.fetchFundHouse(self.investmentAccount().value()).done(function(data) {
            ...
            DetailedDividendReport.fetchAllFundHouse().done(function(data) {
        ...
        self.fundHouseChange = function(event) {
          ...
          DetailedDividendReport.fetchSchemeName(self.investmentAccount().value(), event.detail.value).done(function(data) {
            self.schemesDataLoaded(false);
        ...
        self.searchReports = function() {
          if (self.investmentAccountData() || self.fromDate() || self.toDate() || self.fundHouseData() || self.schemesData()) {
            DetailedDividendReport.fetchDividendReports(self.investmentAccount().value(), self.fundHouseCode(), self.schemeCode(), self.fromDate(), self.toDate()).done(function(data) {
              tempData = null;

              tempData = $.map(data.dividendReportsDTO.dividends, function(v) {
                const newObj = {};
                ...
                newObj.currencyNav = v.schemeNav.currency;
                newObj.dividendAmount = v.txnAmount.amount;
                newObj.currency = v.txnAmount.currency;
                newObj.startSeqNumber = data.dividendReportsDTO.transactionReportId;
                newObj.moreIndicator = data.dividendReportsDTO.moreIndicator;
        ...
        self.download = function() {
          ...
          if (self.investmentAccountData() || self.fromDate() || self.toDate() || self.fundHouseData() || self.schemesData()) {
            DetailedDividendReport.fetchPDF(self.investmentAccount().value(), self.fundHouseCode(), self.schemeCode(), self.fromDate(), self.toDate());
          }
        };
