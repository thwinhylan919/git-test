define([
  ...
  const vm = function(params) {
    ...
    self.termsAndConditionsArray = ko.observableArray();
    self.datasourceForTermsAndConditions = ko.observableArray();
    self.termsAndCondsDataSourceLoaded = ko.observable(true);

    if (params.rootModel.params.guaranteeDetails && params.rootModel.params.guaranteeDetails.termsAndConditions && params.rootModel.params.guaranteeDetails.termsAndConditions.length > 0) {
      for (let i = 0; i < params.rootModel.params.guaranteeDetails.termsAndConditions.length; i++) {
        self.termsAndConditionsArray.push({
          id: ko.observable(i),
          description: ko.observable(params.rootModel.params.guaranteeDetails.termsAndConditions[i].description)
        });
      }
    } else {
      self.termsAndConditionsArray.push({
    ...
    self.stages = [{
      ...
      },
      {
        ...
        editable: ko.observable(true),
        validated: ko.observable(),
        moduleName: "bank-guarantee-terms-and-conditions",
        disabled: ko.observable(false)
      },
      {
    ...
    params.baseModel.registerComponent("bank-guarantee-terms-and-conditions", "guarantee");
    ...
    self.createModelFromArray = function() {
      self.guaranteeDetails.attachedDocuments(self.attachedDocuments());
      ...
      self.guaranteeDetails.termsAndConditions.removeAll();

      for (let i = 0; i < self.termsAndConditionsArray().length; i++) {
        self.guaranteeDetails.termsAndConditions.push({
          serialNo: i+1,
          undertakingType: "GUARANTEE",
          description: self.termsAndConditionsArray()[i].description
        });
      }
    };
    ...
    self.fetchLists = function() {
      GuaranteeModel.fetchProduct().done(function(productData) {
        ...
        if (productData.bgProductDTOList[0].guaranteeTypeConfig) {
          if (productData.bgProductDTOList[0].guaranteeTypeConfig === "Y") {
            ...
            self.guaranteeTypeOptions = ko.observableArray([{
              label: self.resourceBundle.guaranteeDetails.labels.FINANCIAL,
              value: "FINANCIAL"
            }, {
              label: self.resourceBundle.guaranteeDetails.labels.PERFORMANCE,
              value: "PERFORMANCE"
            }]);
          }
        }
    ...
    if (self.mode() === "EDIT") {
      ...
      if (self.guaranteeDetails.attachedDocuments() && self.guaranteeDetails.attachedDocuments().length > 0 && self.guaranteeDetails.attachedDocuments()[0].documentName) {
        self.attachedDocuments(ko.mapping.toJS(self.guaranteeDetails.attachedDocuments()));
      }
    }
