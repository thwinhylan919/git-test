define([
  ...
  const vm = function (params) {
    ...
    },
    {
      ...
      expanded: ko.observable(false),
      templateName: ko.observable("trade-finance/view-guarantees/bank-guarantee-terms-and-conditions")
    }
    ];
    ...
    if (self.setMenuAsAmendment()) {
      ...
      fetchAmendments();
      self.showSection(self.sectionName(), "trade-finance/view-lc/amendmentstrade-finance/view-guarantees/bank-guarantee-amendments");
    }
    ...
    if (self.guaranteeDetails) {
      ...
      ViewBGModel.fetchBeniCountry().done(function (data) {
        ...
        const beneCountry = countryList.filter(function (data) {
          ...
          appCountry = countryList.filter(function (data) {
            return data.code === self.guaranteeDetails.partyAddress.country;
          });

        self.applicantAddress.country(appCountry[0].description);
      ...
      if (self.guaranteeDetails.attachedDocuments) {
        ...
        for (let k = 0; k < self.guaranteeDetails.attachedDocuments.length; k++) {
      ...
      if (self.guaranteeDetails.termsAndConditions && self.guaranteeDetails.termsAndConditions.length > 0) {
        for (let i = 0; i < self.guaranteeDetails.termsAndConditions.length; i++) {
          self.termsAndConditionsListArray.push({
            ...
            description: self.guaranteeDetails.termsAndConditions[i].description
          });
        }
      }
