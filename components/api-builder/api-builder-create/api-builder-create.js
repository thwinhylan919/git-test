define([
    ...
    "./model",
    "ojL10n!resources/nls/api-builder-create",
    "ojs/ojselectcombobox",
    "ojs/ojvalidationgroup",
    "ojs/ojbutton",
    "ojs/ojknockout-validation",
    "ojs/ojradioset"
], function(ko, APIBuilderCreateModel, resourceBundle) {
    "use strict";

    return function(rootParams) {
        const self = this,
            getNewKoModel = function() {
                const KoModel = ko.mapping.fromJS(APIBuilderCreateModel.getNewModel());
        ...
        self.redactionFile = ko.observable();
        self.bussPolicyFile = ko.observable();
        ...
        if (rootParams.rootModel.params.apiServiceDTO) {
            ...
            if (details.responseRedactionType()) {
                self.selectedRedaction(details.responseRedactionType());
            }
            ...
            if (details.moduleName()) {
                ...
                APIBuilderCreateModel.fetchCategoryName(self.selectedModule()).done(function(data) {
                    self.categoryName([]);
        ...
        APIBuilderCreateModel.getApiGroups().then(function(data) {
        ...
        APIBuilderCreateModel.fetchModuleName().done(function(data) {
            if (data.enumRepresentations) {
                for (let i = 0; i < data.enumRepresentations[0].data.length; i++) {
        ...
        APIBuilderCreateModel.fetchTransactionType().done(function(data) {
            if (data.enumRepresentations) {
                for (let i = 0; i < data.enumRepresentations[0].data.length; i++) {
        ...
        APIBuilderCreateModel.fetchActionType().done(function(data) {
            if (data.enumRepresentations) {
                for (let i = 0; i < data.enumRepresentations[0].data.length; i++) {
        ...
        APIBuilderCreateModel.fetchTaskAspect().done(function(data) {
            if (data.enumRepresentations) {
                for (let i = 0; i < data.enumRepresentations[0].data.length; i++) {
        ...
        APIBuilderCreateModel.fetchRedactionType().done(function(data) {
            if (data.enumRepresentations) {
                self.selectedRedaction(data.enumRepresentations[0].data[0].code);
        ...
        self.moduleChangeHandler = function(event) {
            ...
            APIBuilderCreateModel.fetchCategoryName(event.detail.value).done(function(data) {
                self.categoryName([]);
        ...
        self.upload = function() {
            if (self.selectedRedaction() === "JAVA") {
                if (document.getElementById("inputRedaction").files[0]) {
                    self.redactionFile(document.getElementById("inputRedaction").files[0]);
                } else {
                    rootParams.baseModel.showMessages(null, [self.resource.noFileFoundErrorMessage], "INFO");
                }
            }

            if (self.uploadGroovyClass()) {
                if (document.getElementById("inputBussPolicy").files[0]) {
                    self.bussPolicyFile(document.getElementById("inputBussPolicy").files[0]);
                } else {
                    rootParams.baseModel.showMessages(null, [self.resource.noFileFoundErrorMessage], "INFO");
                }
            }
        ...
        self.save = function() {
            if (rootParams.baseModel.showComponentValidationErrors(document.getElementById("tracker"))) {
                ...
                self.apiServiceDTO.responseRedactionType(self.selectedRedaction());
                ...
                self.apiServiceDTO.responseRedaction(self.redactionFile());
                self.apiServiceDTO.refBusinessPolicy(self.bussPolicyFile());
