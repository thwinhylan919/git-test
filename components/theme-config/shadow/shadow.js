    ...
    return function(rootParams) {
        ...
        self.updateColorBox = function(color) {
            ...
            if (index === -1) {
                ...
                self.selectedColors().splice(0, 0, {
                    color: color,
                    count: 1
                });
            }
        ...
        self.showPaleteBox = function() {
            if (!self.readOnly) {
                if (self.bindvariable[rootParams.rootModel.shadowColor]()) {
                    self.colorValue(new oj.Color(self.bindvariable[rootParams.rootModel.shadowColor]()));
                }
        ...
        self.saveSelectedColor = function() {
            self.bindvariable[rootParams.rootModel.shadowColor](self.colorValue().toString());
            setCSSProps(rootParams.rootModel.shadowColor, self.colorValue().toString());
        ...
        if (!self.readOnly) {
            bindDispose = self.bindvariable[rootParams.rootModel.shadowColor].subscribe(function(newValue) {
                setCSSProps(rootParams.rootModel.shadowColor, newValue);
            });
        }
