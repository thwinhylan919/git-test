define([
  ...
  return function (rootParams) {
                ...
                if (data.recordDetails) {
                    for (let i = 0; i < data.recordDetails.length; i++) {
                        self.recordData = {
                            ...
                            debitAccountBranchCode: ko.observable(),
                            creditAccountId: ko.observable(),
                            creditAccountBranchCode: ko.observable(),
                            transaction: ko.observable(),
                            paymentType: ko.observable(),
                            date: ko.observable(),
                            beneficiaryName: ko.observable(),
                            auditSequence: ko.observable(),
                            fileRefId: ko.observable(),
                            generatedPackageId: ko.observable(),
                            recRefId: ko.observable(),
                            status: ko.observable(),
                            valueDate: ko.observable(),
                            version: ko.observable(),
                            accountingType: ko.observable()
                        };

                        self.recordData.amount(rootParams.baseModel.formatCurrency(data.recordDetails[i].currencyAmount.amount, data.recordDetails[i].currencyAmount.currency));
                        self.recordData.statusDesc(self.rStatusListMap[data.recordDetails[i].status]);

                        if (rootParams.rootModel.params[3]) {
                            self.recordData.accountingType(rootParams.rootModel.params[0].accountingType);
                        }

                        if (data.recordDetails[i].debitAccountId) {
                            self.recordData.debitAccountId(data.recordDetails[i].debitAccountId);
                            self.recordData.debitAccountBranchCode(data.recordDetails[i].debitAccountId.substring(0, 3));
                        }

                        if (data.recordDetails[i].creditAccountId) {
                            self.recordData.creditAccountId(data.recordDetails[i].creditAccountId);
                            self.recordData.creditAccountBranchCode(data.recordDetails[i].creditAccountId.substring(0, 3));
                        }

                        self.recordData.transaction(data.recordDetails[i].paymentType);
                        self.recordData.paymentType(self.transactionTypesMap[data.recordDetails[i].paymentType]);

                        if (data.recordDetails[i].valueDate) {
                            self.recordData.date(data.recordDetails[i].valueDate);
                        }

                        if (!data.recordDetails[i].beneficiaryName) {
                            self.recordData.beneficiaryName("-");
                        } else {
                            self.recordData.beneficiaryName(data.recordDetails[i].beneficiaryName);
                        }

                        self.recordData.fileRefId(data.recordDetails[i].fileRefId);
                        self.recordData.recRefId(data.recordDetails[i].recRefId);
                        self.recordData.auditSequence(data.recordDetails[i].auditSequence);
                        self.recordData.generatedPackageId(data.recordDetails[i].generatedPackageId);
                        self.recordData.status(data.recordDetails[i].status);
                        self.recordData.valueDate(data.recordDetails[i].valueDate);
                        self.recordData.version(data.recordDetails[i].version);
                        self.recordDetailsMap[data.recordDetails[i].recRefId] = ko.mapping.toJS(self.recordData);
                        self.recordDetails.push(ko.mapping.toJS(self.recordData));
    ...
    self.renderCheckBox = function (context) {
      ...
      if (context.row.status !== "PROCESSING_IN_PROGRESS" || context.row.accountingType === "SDMC") {
