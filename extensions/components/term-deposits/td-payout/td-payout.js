  ...
  return function (rootParams) {
    ...
    payoutInstructionsModel.fetchTransferOption().then(function (data) {
      ...
      for (let index = 0; index < data.enumRepresentations[0].data.length; index++) {
        if (data.enumRepresentations[0].data[index].code !== "E" && data.enumRepresentations[0].data[index].code !== "INT") {
          tempData.push(data.enumRepresentations[0].data[index]);
        }

      }

      transferOptions = tempData;
      ko.utils.arrayPushAll(self.transferOptionsList, tempData);
