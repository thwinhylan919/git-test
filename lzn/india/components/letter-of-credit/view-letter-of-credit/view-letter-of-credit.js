define([
  ...
  const vm = function (params) {
    ...
    self.populateGoodsLists = function () {
      if (self.letterOfCreditDetails.goods && self.letterOfCreditDetails.goods.length > 0) {
        ...
        for (i = 0; i < self.letterOfCreditDetails.goods.length; i++) {
          ...
          self.goodsLists.push({
            ...
            underLicense: self.letterOfCreditDetails.goods[i].underLicense ? ko.observable(self.letterOfCreditDetails.goods[i].underLicense) : ko.observable(false),
            datasourceForLicense: ko.observable(new oj.ArrayTableDataSource(licenseArray, { idAttribute: "licenseNumber" }))
          });
        }
