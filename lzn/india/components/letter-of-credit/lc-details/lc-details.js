define([
  ...
  const vm = function (params) {
    ...
    if (self.requestedConfirmationPartyOptions() === undefined || self.requestedConfirmationPartyOptions().length === 0) {
    ...
    self.confirmationInstructionChangeHandler = function (event) {
      ...
      self.confirmationInstructionOptions().forEach(function (element) {
        if (self.confirmationInstruction() === element.value) {
          self.letterOfCreditDetails.confirmationInstruction(element.value);
        }
      });

      if (self.confirmationInstruction() !== "WITHOUT") {
    ...
    self.requestedConfirmationPartyChangeHandler = function (event) {
      ...
      self.requestedConfirmationPartyOptions().forEach(function (element) {
        if (self.requestedConfirmationParty() === element.value) {
          self.letterOfCreditDetails.requestedConfirmationParty(element.value);
        }
      });

      if (self.requestedConfirmationParty() !== "ABK") {
      ...
      if (trackerSwift.valid === "valid") {
        if (!self.bicCodeError()) {
          LCDetailsModel.getBankDetailsBIC(self.confirmationInstructionBankCode()).done(function (data) {
            ...
            self.letterOfCreditDetails.requestedConfirmationPartyDetails = data;
          });
        }
      } else {
    ...
    function fetchAdditionalDetails() {
      if (self.bankAddressOne() === null && self.letterOfCreditDetails.availableWith() !== null && self.letterOfCreditDetails.availableWith() !== "") {
    ...
    self.availableWithSubscribe = self.creditAvailableWithSelected.subscribe(function (newValue) {
      if (newValue) {
        self.letterOfCreditDetails.availableWith(null);
        self.availableWithDetails(null);
        self.letterOfCreditDetails.bankAddress(null);
        self.bankAddressOne(null);
        self.bankAddressTwo(null);
        self.bankAddressThree(null);
      }
    });
    ...
    if (self.transactionType === "SHIPPING_GUARANTEE") {
      ...
      if (ko.utils.unwrapObservable(self.letterOfCreditDetails.amount) && ko.utils.unwrapObservable(self.letterOfCreditDetails.amount
      ).currency) {
