...
], function(ko, $, ReviewService, resourceBundle) {
  "use strict";
  ...
  return function(rootParams) {
    ...
    self.searchEditableFields = function(obj) {
      ...
      for (const key in obj) {
        if (key.search("isEditable") > -1 && obj[key]()) {
          return true;
        }
      }
    ...
    self.submitApplicationSuccessHandler = function(event) {
      self.productDetails().sectionBeingEdited("");

      if (event.applicationDetailsDTO[0].applicationId && event.applicationDetailsDTO[0].applicationId.value) {
        self.productDetails().applicationId.value = event.applicationDetailsDTO[0].applicationId.value;
        self.productDetails().applicationId.displayValue = event.applicationDetailsDTO[0].applicationId.displayValue;
        self.appRefNo(event.applicationDetailsDTO[0].applicationId.displayValue);
      }
    ...
    self.submitApplication = function() {
      ...
      if (tracker.valid === "valid") {
        if (self.userType === "primary") {
          ...
          ReviewService.fetchRegistrationRequired(self.productDetails().submissionId.value).done(function(data) {
