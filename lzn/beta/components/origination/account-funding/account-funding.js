define([
  ...
  return function(rootParams) {
    ...
    self.displayContinueButton = ko.observable(false);
    ...
    self.getAccountFundingList = function(data, event) {
      ...
      if (eventValue && eventValue !== null) {
        ...
        if (eventValue === "COLL" && !self.linkedAccountListLoaded() && count === 0) {
          count = count + 1;
        ...
        if (eventValue === "DDAO" && !self.casaOwnAccountListLoaded() && count === 0) {
          count = count + 1;
        ...
        if ((eventValue === "CARD_CREDIT" || eventValue === "CARD_DEBIT") && !self.cardFormatsListListLoaded() && count === 0) {
          count = count + 1;
          ...
          if (data.accountConfigDTO) {
            if (data.accountConfigDTO.termDepositApplicationRequirementDTO) {
              if (data.accountConfigDTO.termDepositApplicationRequirementDTO.requestedAmount) {
                ...
                self.productDetails().requirements.requestedAmount.amount(data.accountConfigDTO.termDepositApplicationRequirementDTO.requestedAmount.amount);
              }
              ...
              if (data.accountConfigDTO.termDepositApplicationRequirementDTO.requestedTenure) {
                ...
                self.productDetails().requirements.requestedTenure.months(data.accountConfigDTO.termDepositApplicationRequirementDTO.requestedTenure.months);
                self.productDetails().requirements.requestedTenure.years(data.accountConfigDTO.termDepositApplicationRequirementDTO.requestedTenure.years);
              ...
              if (data.accountConfigDTO.termDepositApplicationRequirementDTO.frequency) {
                ...
                self.applicantObject().accountFunding.tdRequirements.termDepositApplicationRequirementDTO.interestCompoundingFrequency = data.accountConfigDTO.termDepositApplicationRequirementDTO.interestCompoundingFrequency;
            ...
            if (data.accountConfigDTO.settlementMode) {
              ...
              if (self.productDetails().productClassName === "TERM_DEPOSITS" && data.accountConfigDTO.settlementMode.settlementType !== "DDAO" && data.accountConfigDTO.settlementMode.settlementType !== "COLL" && data.accountConfigDTO.settlementMode.settlementType !== "CARD") {
    ...
    self.showWhatIsThisText = function(data) {
      if (data === "CARD_CREDIT") {
        $("#whatisThisCredit").show().trigger("openModal");
      }

      if (data === "CARD_DEBIT") {
        $("#whatisThisDebit").show().trigger("openModal");
      }
    };

    self.completeAccountFundingSection = function() {
      self.completeApplicationStageSection(rootParams.applicantStages, rootParams.applicantAccordion, "account-funding");
    };

    self.createTDRequirements = function(event) {
      if (event.detail.value) {
        self.displayContinueButton(true);

        const tdReq = ko.mapping.toJS(ko.mapping.fromJS(self.applicantObject().accountFunding.tdRequirements));

        tdReq.termDepositApplicationRequirementDTO.requestedTenure.years = parseInt(self.applicantObject().accountFunding.tdRequirements.termDepositApplicationRequirementDTO.requestedTenure.years());
        tdReq.termDepositApplicationRequirementDTO.requestedTenure.months = parseInt(self.applicantObject().accountFunding.tdRequirements.termDepositApplicationRequirementDTO.requestedTenure.months());
        tdReq.termDepositApplicationRequirementDTO.requestedTenure.days = parseInt(self.applicantObject().accountFunding.tdRequirements.termDepositApplicationRequirementDTO.requestedTenure.days());
        tdReq.termDepositApplicationRequirementDTO.frequency = event.detail.value;
        self.applicantObject().accountFunding.tdRequirements.termDepositApplicationRequirementDTO.frequency = tdReq.termDepositApplicationRequirementDTO.frequency;
        ...
        AccountFundingModel.createTDRequirements(payLoad).done(function(data) {
          ...
          AccountFundingModel.getExistingAccountConfig().done(function(data) {
            if (data.accountConfigDTO.maturityInformationDTO) {
              self.applicantObject().accountFunding.interestRate(data.accountConfigDTO.maturityInformationDTO.annualEquivalentRate);
              ko.tasks.runEarly();
              self.displayContinueButton(false);
            }
          });
        });
      }
    ...
    self.linkedAccountChanged = function(event) {
      ...
      if (event.detail.value) {
        ...
        for (let i = 0; i < self.casaOwnAccountList().length; i++) {
    ...
    self.saveAccountFundingInfo = function(event, data) {
      ...
      $("#SAVEFUNDINGSOURCE").hide().trigger("closeModal");
    ...
    self.calculateLoanAmount = ko.computed(function() {
      if (self.productDetails().productClassName === "TERM_DEPOSITS") {
        self.productDetails().requirements.requestedAmount.amount(self.applicantObject().accountFunding.tdRequirements.termDepositApplicationRequirementDTO.requestedAmount.amount());
        self.productDetails().requirements.requestedTenure.months(parseInt(self.applicantObject().accountFunding.tdRequirements.termDepositApplicationRequirementDTO.requestedTenure.months()));
        self.productDetails().requirements.requestedTenure.years(parseInt(self.applicantObject().accountFunding.tdRequirements.termDepositApplicationRequirementDTO.requestedTenure.years()));
        self.productDetails().requirements.requestedTenure.days = parseInt(self.applicantObject().accountFunding.tdRequirements.termDepositApplicationRequirementDTO.requestedTenure.days());
