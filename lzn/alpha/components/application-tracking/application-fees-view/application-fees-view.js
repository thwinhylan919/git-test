define([
  ...
  return function(rootParams) {
    ...
    self.sumCurrency=ko.observable();
        ...
        if (self.feesConfigured()) {
          for (let i = 0; i < self.bankFees().fundingTableDetailDTO[0].fundingItemDetailDTOs[bankFeeIndex].bankFeeFundingItemDTOs.length; i++) {
            self.applicationFees()[i] = {
              ...
              amount: self.bankFees().fundingTableDetailDTO[0].fundingItemDetailDTOs[bankFeeIndex].bankFeeFundingItemDTOs[i].totalFeeValue.amount,
              currency: self.bankFees().fundingTableDetailDTO[0].fundingItemDetailDTOs[bankFeeIndex].bankFeeFundingItemDTOs[i].totalFeeValue.currency
            };
          }

          self.sumAmount(self.bankFees().fundingTableDetailDTO[0].fundingItemDetailDTOs[bankFeeIndex].totalAmount.amount());
          self.sumCurrency(self.bankFees().fundingTableDetailDTO[0].fundingItemDetailDTOs[bankFeeIndex].totalAmount.currency);
        }
