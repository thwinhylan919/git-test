define([
  ...
  return function AccountFundingModel() {
    ...
    const fetchFrequencyList = function(data, deferred) {
      const options = {
        url: "submissions/{submissionId}/depositApplications/interestFrequencies?currency={baseCurrency}&amount={requestedAmount}&days={requestedTenureDays}&months={requestedTenureMonths}&years={requestedTenureYears}&offerID={offerId}&productCode={productCode}&productGroupSerialNumber={productGroupSerialNumber}",
        success: function(data) {
          deferred.resolve(data);
        },
        error: function(data) {
      ...
      params = {
        ...
        requestedTenureDays: data.requirements.requestedTenure.days,
